...
- Never use `get_url` to write version strings or other small text metadata to `/tmp/` (or any temp path) and then read them back via `lookup('file', ...)`. Use `ansible.builtin.uri` with `return_content: true` + `register` instead — the version data lives in a variable, not on disk. This avoids permission races and stale-file bugs.
